function init({
  ...
  const micButton = micButtonId ? document.getElementById(micButtonId) : null;
  ...
  let W, H, N;
  let img, pixels;
  let zField, energyField, newEnergy, sand;
  let sinMx, sinNx, sinNy, sinMy;
  let px, py, vx, vy;
  ...
  const BASE_PARTICLE_COUNT = 14000;
  const PARTICLE_DENSITY = BASE_PARTICLE_COUNT / (BASE_WIDTH * BASE_HEIGHT);
  ...
  function disconnectCurrentSource(nextSource = null) {
  ...
  async function useFile() {
    ...
    disconnectCurrentSource(fileSource);
  ...
  async function useMic() {
    ...
    try {
      ...
      disconnectCurrentSource(micSource);
  ...
  if (micButton) {
    ...
    console.warn(`Mic button not found: ${micButtonId}`);
  }
  ...
  function resizeCanvas() {
    ...
    const targetW =
      width ??
      (autoFit && parent ? parent.clientWidth : canvas.width || DEFAULT_SIZE);

    const targetH =
      height ??
      (autoFit && parent ? parent.clientHeight : canvas.height || DEFAULT_SIZE);

    const nextW = Math.max(1, Math.floor(targetW || DEFAULT_SIZE));
    const nextH = Math.max(1, Math.floor(targetH || DEFAULT_SIZE));
    ...
    zField = new Float32Array(N);
    energyField = new Float32Array(N);
    newEnergy = new Float32Array(N);
    sand = new Float32Array(N);

    sinMx = new Float32Array(W);
    sinNx = new Float32Array(W);
    sinNy = new Float32Array(H);
    sinMy = new Float32Array(H);

    PARTICLE_COUNT = Math.max(
      500,
      Math.floor(W * H * PARTICLE_DENSITY * particleDensityMultiplier)
    );
  ...
  function updateParticlesAndSand() {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      sand[(nx | 0) + (ny | 0) * W] += DEPOSIT * depositBoost;
    }
  }
  ...
  if (autoFit && width == null && height == null) {
  ...
  return {
    ...
    destroy() {
      try {
        ...
        if (fileInput) fileInput.removeEventListener("change", useFile);
        if (micButton) micButton.removeEventListener("click", useMic);
